...
def test_pericarpium_zanthoxyli_maps_to_seasoning() -> None:
    assert categorize_item("FLOWER PERICARPIURN ZANTHOXYLI") == "Expenses:Food:Grocery:Seasoning"


def test_red_chili_pepper_maps_to_seasoning() -> None:
    assert categorize_item("T&T SLICED RED CHILI PEPPER") == "Expenses:Food:Grocery:Seasoning"


def test_coors_maps_to_alcoholic_beverage() -> None:
    assert categorize_item("COORS LIGHT 6 PK HQ") == "Expenses:Food:AlcoholicBeverage"
